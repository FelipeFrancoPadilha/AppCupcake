...
const Cart: React.FC<CartProps> = ({ items, allProducts, onClose, onUpdateQuantity, onRemove, onCheckout, onAddToCart }) => {
      ...
      <div className="absolute inset-y-0 right-0 w-full max-w-md bg-rose-100 shadow-xl flex flex-col">
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-rose-200 bg-rose-50">
            {upsellProduct && (
              <div className="bg-rose-100 border-2 border-dashed border-pink-300 rounded-lg p-3 mb-4">
                <h4 className="text-sm font-bold text-pink-700 mb-2 text-center">✨ Você também pode gostar! ✨</h4>
                <div className="flex items-center gap-3">
                  <img src={upsellProduct.imagem} alt={upsellProduct.nome} className="w-16 h-16 rounded-md object-cover" />
                  <div className="flex-grow">
                    <p className="font-semibold text-gray-800 text-sm">{upsellProduct.nome}</p>
                    <p className="text-pink-600 font-bold text-sm">R$ {upsellProduct.preço}</p>
                  </div>
                  <button 
                    onClick={() => onAddToCart(upsellProduct.id)}
                    className="bg-pink-500 text-white font-bold text-xs py-2 px-3 rounded-md hover:bg-pink-600 transition-colors"
                    aria-label={`Adicionar ${upsellProduct.nome} ao carrinho`}
                  >
                    Adicionar
                  </button>
                </div>
